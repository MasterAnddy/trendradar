...
class NewsAnalyzer:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            print(f"分析流程执行出错: {e}")
            # A scheduled CI run must fail at the real analysis error instead
            # of continuing to the persistence step and reporting a misleading
            # git failure. Preserve the existing forgiving local CLI behavior.
...
"""
    ...
    try:
        ...
        print(f"❌ 程序运行错误: {e}")
        if debug_mode or os.environ.get("GITHUB_ACTIONS", "").lower() == "true":
            raise
...
def _handle_status_commands(config: Dict) -> None:
    ...
    try:
        ...
        print("\n" + "=" * 60)
        # 清理资源 even when CI propagates a configuration failure.
        ctx.cleanup()
